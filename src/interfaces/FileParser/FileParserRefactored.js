...
function parseNode(graph, tokens) {
    ...
    if ( isNumeric(tokens[4]) ) {
        attributes["weight"] = parseFloat(tokens[4]);
    }
    const attributes = parseAttributes(tokens, (attributes["weight"] === undefined) ? 4 : 5);
...
function parseEdge(graph, tokens) {
    ...
    const attributes = parseAttributes(tokens, (attributes["weight"] === undefined) ? 3 : 4);
